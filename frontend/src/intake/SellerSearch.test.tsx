...
describe("SellerSearch", () => {
  ...
  it("renders search input and empty state", () => {
    ...
    expect(
      screen.getByRole("button", { name: /register new seller/i }),
    ).toBeInTheDocument();
  });
  ...
  it("shows results matching query", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText("Jane Doe — A001 · Individual"),
      ).toBeInTheDocument(),
    );
  });
  ...
  it("shows the type tag and vendor-aware name per result", async () => {
    ...
    server.use(http.get("/sellers", () => HttpResponse.json([SELLER, vendor])));
    render(<SellerSearch onSelect={vi.fn()} onCreateNew={vi.fn()} />);
    fireEvent.change(screen.getByPlaceholderText(/search by name or code/i), {
      target: { value: "a" },
    });
    ...
    expect(
      screen.getByText("Acme Outdoors — VCO1 · Vendor"),
    ).toBeInTheDocument();
  });
