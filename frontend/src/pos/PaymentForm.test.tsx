...
describe("PaymentForm", () => {
  ...
  it("shows error when paying by card without a transaction id", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /complete sale/i }));
    expect(screen.getByRole("alert")).toHaveTextContent(
      /card transaction id is required/i,
    );
    expect(onSubmit).not.toHaveBeenCalled();
  });
  ...
  it("submits manual card payment with transaction id", () => {
    ...
    fireEvent.change(screen.getByLabelText(/card transaction id/i), {
      target: { value: "tc_9k2" },
    });
    fireEvent.click(screen.getByRole("button", { name: /complete sale/i }));
    expect(onSubmit).toHaveBeenCalledWith({
  ...
  /** Verifies the card option is hidden entirely when cardEnabled is false (Square integration unfinished). */
  it("hides card fields when cardEnabled is false", () => {
    render(
      <PaymentForm
        total={115}
        onSubmit={vi.fn()}
    ...
    // Cash and check remain available.
    expect(screen.getByLabelText(/cash/i)).toBeInTheDocument();
    expect(screen.getByLabelText(/check/i)).toBeInTheDocument();
  });

  /** Verifies a cash-only submission with card disabled sends square: 0 and no card data. */
  it("submits cash-only payment when card is disabled", () => {
    const onSubmit = vi.fn();
    render(
      <PaymentForm
        total={115}
