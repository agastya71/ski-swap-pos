...
const ITEM_MULTI: ItemLookupResponse = {
  ...ITEM_A,
  id: 2,
  code: "A001-002",
  quantity: 5,
...
const SALE: SaleWithItemsResponse = {
  id: 1,
  event_id: 1,
  date_of_sale: "2026-04-04T10:00:00",
  customer_name: null,
  customer_email: null,
  sale_total: 75,
  mysl_total: 22.5,
...
function scan(code: string) {
  ...
  fireEvent.keyDown(screen.getByPlaceholderText(/scan barcode/i), {
    key: "Enter",
  });
}
...
describe("POSPage", () => {
  ...
  it("increments line quantity when the same item is scanned twice", async () => {
    ...
    await waitFor(() =>
      expect(
        (screen.getByLabelText(/quantity for A001-002/i) as HTMLInputElement)
          .value,
      ).toBe("2"),
    );
  });
  ...
  /** Verifies the active event name is shown on the checkout (header + completed-transaction receipt). */
  it("displays the active event on the header and the completed transaction", async () => {
    renderPOS();
    // Header fetches /events/active (default mock → "Swap 2026").
    await waitFor(() =>
      expect(screen.getByText(/checkout — swap 2026/i)).toBeInTheDocument(),
    );
    ...
    // The transaction receipt carries the event name.
    expect(screen.getByText(/event:/i)).toHaveTextContent("Swap 2026");
  });
  ...
  it("restores the receipt after reload instead of an editable cart", () => {
    ...
    expect(
      screen.queryByPlaceholderText(/scan barcode/i),
    ).not.toBeInTheDocument();
    // The restored receipt is genuinely read-only: no qty/price inputs, no Remove.
    expect(
      screen.queryByLabelText(/quantity for A001-001/i),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByLabelText(/unit price for A001-001/i),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /remove A001-001/i }),
    ).not.toBeInTheDocument();
  });
