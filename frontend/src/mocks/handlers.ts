...
export const handlers = [
    ...
    http.get("/auth/generate-password", () =>
        HttpResponse.json({ password: "Generated1!" }),
    ),
    ...
    /** GET /events/active — the active event, for display on POS/intake screens. */
    http.get("/events/active", () =>
        HttpResponse.json({
            id: 2,
            name: "Swap 2026",
            year: 2026,
    ...
    /** GET /items/intake-search — full-field intake item search (empty by default; tests override). */
    http.get("/items/intake-search", () => HttpResponse.json([])),
    ...
    http.get("/sellers", ({ request }) => {
        // request.url is always a valid absolute URL from the MSW/Fetch machinery.
        // Parsed via URLSearchParams (never throws) instead of new URL() (can throw).
        const q =
        ...
        const sellers = [
            ...
            (s) =>
                !q ||
                s.code.includes(q) ||
                s.first_name.toLowerCase().includes(q.toLowerCase()) ||
    ...
    http.post("/intakes/:id/labels", () =>
        HttpResponse.json({ intake_id: 1, printed: 5 }),
    ),
    ...
    http.get("/items/lookup", () =>
        HttpResponse.json({ ...ITEM, seller_code: "A001" }),
    ),
    ...
    http.post("/sales/:id/void", () =>
        HttpResponse.json({ ...SALE, is_voided: true }),
    ),

    /** GET /reports/:eventId/seller/:sellerId — seller payout (sales + unsold sections + seller info). */
    http.get("/reports/:eventId/seller/:sellerId", () =>
        HttpResponse.json({
            event_id: 1,
            event_name: "Swap 2026",
            seller_id: 1,
            seller_code: "A001",
            ...
            sales: [
                {
                    item_code: "A001-1",
                    description: "Atomic skis",
                    date_of_sale: "2026-04-04T10:00:00Z",
                    quantity_sold: 1,
                    sell_price: 120.0,
    ...
    /** GET /reports/:eventId/sellers-payouts — batch payouts for ALL sellers. */
    http.get("/reports/:eventId/sellers-payouts", () =>
        HttpResponse.json({
            event_id: 1,
            event_name: "Swap 2026",
            seller_count: 1,
            gross_sales_total: 120.0,
            mysl_total: 36.0,
            seller_total: 84.0,
            sellers: [
                {
    ...
    http.post(
        "/admin/backup",
        () =>
            new HttpResponse(new Blob(["zip"], { type: "application/zip" }), {
                status: 200,
            }),
    ),
];
