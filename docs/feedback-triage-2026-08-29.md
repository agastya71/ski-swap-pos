...
| 🔧 Follow-up (from independent PR review, pre-existing on `main`, not blocking): clearing the Price field in the item edit panels (ItemList/SellerDetailPage `handleItemSave`) can save NaN → NOT NULL integrity → 500. Harden those save handlers in a future PR (guard `parseFloat` + treat blank as "don't change"). |
